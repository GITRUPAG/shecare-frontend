...
export default function DashboardPage() {
  ...
  // API state — all start as null; 403s for new users are handled gracefully
  ...
  // ── FIX: Promise.allSettled means a 403 on one endpoint never crashes the page ──
  useEffect(() => {
    const fetchAll = async () => {
      ...
      // Only set state when the call actually succeeded
  ...
  // ── Calendar markers — all guarded with optional chaining ─────────────────
  ...
  // FIX: every calendar field accessed with ?. so null calendar never crashes
  ...
  // ── Stat cards — FIX: all calendar fields use ?. so null never crashes ────
  const nextPeriodStr = fmt(calendar?.nextPeriodStart);           // fmt() handles null → "—"
  const daysToNext    = daysUntil(calendar?.nextPeriodStart);     // daysUntil() handles null → null
  const currentPhase  = calendar?.currentPhase ?? "—";
  const cycleLength   = prediction?.predictedCycleLength
    ? `${Math.round(prediction.predictedCycleLength)} days` : "—";

  // FIX: stat card sub-label — was `fmt(calendar.nextPeriodStart)` (no ?.), now safe
  const nextPeriodSub = calendar?.nextPeriodStart
    ? `Next: ${fmt(calendar.nextPeriodStart)}` : "Log a period to predict";
  ...
  const insightBg = {
    tip:   { bg:"#FFF5EE", border:"#FFD9C4", c:C.coralDark },
    alert: { bg:"#FFFBEB", border:"#FDE68A", c:"#B45309" },
  };
  ...
  const pcosBadge = pcosRisk === "high"
    ...
    : pcosRisk === "medium"
      ? { bg:"#FEF3C7", c:"#D97706", border:"#FDE68A" }
    ...
    <AppShell current="dashboard">
      ...
      <div style={{ padding:"32px 36px", maxWidth:1100 }}>
        ...
        <div style={{ display:"flex", alignItems:"flex-start", justifyContent:"space-between", marginBottom:32 }}>
          <div>
            <p style={{ fontFamily:"'Nunito', sans-serif", fontSize:13, color:C.textSoft, marginBottom:4 }}>Good morning ✨</p>
            <h1 style={{ fontFamily:"'Cormorant Garamond', serif", fontSize:34, fontWeight:700, color:C.textDark, letterSpacing:"-0.5px" }}>Welcome back, {displayName}</h1>
          </div>
          <button onClick={() => navigate("/tracker")} style={{
            background:`linear-gradient(135deg, ${C.coral}, ${C.coralDark})`, color:"white",
            border:"none", borderRadius:12, padding:"12px 22px",
            fontFamily:"'Nunito', sans-serif", fontSize:14, fontWeight:700,
            cursor:"pointer", boxShadow:"0 4px 16px rgba(255,107,71,0.30)",
          }}>+ Log Today</button>
        </div>
        ...
        {/* ── New user nudge — shown when loading is done and no data exists ── */}
        {!loading && !calendar && !prediction && (
          <div style={{ background:`linear-gradient(135deg, ${C.peachPale}, #fff)`, border:`1.5px solid ${C.peachLight}`, borderRadius:20, padding:"24px 28px", marginBottom:24, display:"flex", alignItems:"center", gap:20 }}>
            <span style={{ fontSize:40 }}>🌸</span>
            <div style={{ flex:1 }}>
              <p style={{ fontFamily:"'Cormorant Garamond', serif", fontSize:20, fontWeight:700, color:C.textDark, marginBottom:4 }}>Start tracking your cycle</p>
              <p style={{ fontFamily:"'Nunito', sans-serif", fontSize:13, color:C.textSoft }}>Log your first period to unlock AI predictions, phase insights and your personal health dashboard.</p>
            </div>
            <button onClick={() => navigate("/tracker")} style={{ background:`linear-gradient(135deg, ${C.coral}, ${C.coralDark})`, color:"white", border:"none", borderRadius:12, padding:"12px 22px", fontFamily:"'Nunito', sans-serif", fontSize:14, fontWeight:700, cursor:"pointer", whiteSpace:"nowrap", boxShadow:"0 4px 16px rgba(255,107,71,0.30)" }}>
              Log First Period →
            </button>
          </div>
        )}
        ...
        <div style={{ display:"grid", gridTemplateColumns:"repeat(4, 1fr)", gap:16, marginBottom:28 }}>
          {loading ? (
            Array.from({ length:4 }).map((_, i) => (
              <div key={i} style={{ borderRadius:20, overflow:"hidden" }}><LoadingCard height={120} /></div>
            ))
          ) : (
            [
              {
                label:"Next Period",
                // FIX: nextPeriodStr already uses ?. via fmt(); safe for null calendar
                val: nextPeriodStr,
                sub: daysToNext != null ? `in ${daysToNext} days` : "No data yet",
                icon:"🩸",
                g:`linear-gradient(135deg, #FF6B47, #E5542F)`,
              },
              {
                label:"Current Phase",
                val: currentPhase,
                // FIX: was `fmt(calendar.nextPeriodStart)` — direct access on possibly-null calendar
                sub: nextPeriodSub,
                icon:"🌘",
                g:`linear-gradient(135deg, #9333EA, #7C3AED)`,
              },
              {
                label:"Cycle Length",
                val: cycleLength,
              ...
              {
                label:"Health Score",
                val: riskScore != null ? `${riskScore}/100` : "—",
                sub: riskDelta != null ? `${riskDelta > 0 ? "+" : ""}${riskDelta} this week` : riskLabel || "Log data to see",
                icon:"❤️",
                g:`linear-gradient(135deg, #22C55E, #16A34A)`,
              },
            ].map(c => (
              <div key={c.label} style={{ background:c.g, borderRadius:20, padding:"22px 20px", color:"white", boxShadow:"0 4px 20px rgba(0,0,0,0.10)" }}>
                <div style={{ fontSize:24, marginBottom:10 }}>{c.icon}</div>
                <p style={{ fontFamily:"'Nunito', sans-serif", fontSize:11, fontWeight:700, opacity:0.75, textTransform:"uppercase", letterSpacing:"0.8px", marginBottom:4 }}>{c.label}</p>
                <p style={{ fontFamily:"'Cormorant Garamond', serif", fontSize:26, fontWeight:700, lineHeight:1 }}>{c.val}</p>
                <p style={{ fontFamily:"'Nunito', sans-serif", fontSize:11, opacity:0.7, marginTop:4 }}>{c.sub}</p>
              </div>
            ))
          )}
        </div>
        ...
        <div style={{ display:"grid", gridTemplateColumns:"1.5fr 1fr", gap:20 }}>
          ...
          {/* Calendar */}
          <div style={{ background:"white", borderRadius:24, padding:"28px", border:`1px solid ${C.peachLight}`, boxShadow:"0 2px 16px rgba(255,107,71,0.06)" }}>
            <div style={{ display:"flex", alignItems:"center", justifyContent:"space-between", marginBottom:24 }}>
              <h2 style={{ fontFamily:"'Cormorant Garamond', serif", fontSize:22, fontWeight:700, color:C.textDark }}>{MONTHS[cm]} {cy}</h2>
              <div style={{ display:"flex", gap:4 }}>
                {["‹","›"].map((a, i) => (
                  <button key={a} onClick={() => {
                    ...
                    onMouseEnter={e => { e.currentTarget.style.background=C.peachPale; e.currentTarget.style.color=C.coral; }}
            ...
            <div style={{ display:"grid", gridTemplateColumns:"repeat(7, 1fr)", marginBottom:8 }}>
              {DAYS.map((d, i) => <div key={i} style={{ textAlign:"center", fontFamily:"'Nunito', sans-serif", fontSize:11, fontWeight:800, color:C.textSoft, padding:"4px" }}>{d}</div>)}
            </div>
            ...
            {/* Legend */}
            <div style={{ display:"flex", flexWrap:"wrap", gap:16, marginTop:20, paddingTop:16, borderTop:`1px solid ${C.peachLight}` }}>
              {[
            ...
            {/* FIX: empty state when no calendar data — previously this section would
                 crash trying to display markers that came from null calendar fields */}
            {!loading && !calendar && (
              <div style={{ marginTop:16, padding:"14px", background:C.peachPale, borderRadius:12, textAlign:"center" }}>
          ...
          <div style={{ display:"flex", flexDirection:"column", gap:16 }}>
            ...
            {/* PCOS risk */}
            <div style={{ background:"white", borderRadius:20, padding:"22px", border:`1px solid ${C.peachLight}`, boxShadow:"0 2px 12px rgba(255,107,71,0.05)" }}>
              <div style={{ display:"flex", alignItems:"center", justifyContent:"space-between", marginBottom:12 }}>
                <h3 style={{ fontFamily:"'Nunito', sans-serif", fontSize:14, fontWeight:700, color:C.textDark }}>🧬 PCOS Risk</h3>
                {loading ? null : (
              ...
              {loading ? <LoadingCard height={24} /> : (
                <>
                  ...
                  <p style={{ fontFamily:"'Nunito', sans-serif", fontSize:12, color:C.textSoft }}>
                    ...
                    <button onClick={() => navigate("/pcos")} style={{ background:"none", border:"none", cursor:"pointer", color:C.coral, fontWeight:700, fontSize:12, fontFamily:"'Nunito', sans-serif" }}>
            ...
            {/* Insights */}
            <div style={{ background:"white", borderRadius:20, padding:"22px", border:`1px solid ${C.peachLight}`, boxShadow:"0 2px 12px rgba(255,107,71,0.05)", flex:1 }}>
              <h3 style={{ fontFamily:"'Nunito', sans-serif", fontSize:14, fontWeight:700, color:C.textDark, marginBottom:16 }}>💡 Today's Insights</h3>
              {loading ? (
                <div style={{ display:"flex", flexDirection:"column", gap:10 }}>
                  {[80, 72, 80].map((h, i) => <LoadingCard key={i} height={h} />)}
                </div>
              ) : (
                <div style={{ display:"flex", flexDirection:"column", gap:10 }}>
                  {displayInsights.map((ins, i) => {
                    ...
                    return (
                      <div key={i} style={{ background:s.bg, border:`1px solid ${s.border}`, borderRadius:12, padding:"12px 14px" }}>
                        <p style={{ fontFamily:"'Nunito', sans-serif", fontSize:13, fontWeight:700, color:C.textDark, marginBottom:4 }}>{ins.icon} {ins.title}</p>
                        <p style={{ fontFamily:"'Nunito', sans-serif", fontSize:12, color:C.textSoft, lineHeight:1.6 }}>{ins.body}</p>
                      </div>
                    );
                  })}
                </div>
        ...
        {/* Cycle Summary — FIX: only rendered when prediction is non-null,
            so calendar.nextPeriodStart etc. are never read on null objects */}
        {!loading && prediction && (
          <div style={{ background:"white", borderRadius:24, padding:"28px", border:`1px solid ${C.peachLight}`, boxShadow:"0 2px 16px rgba(255,107,71,0.06)", marginTop:20 }}>
            <div style={{ display:"flex", alignItems:"center", justifyContent:"space-between", marginBottom:20 }}>
              <h2 style={{ fontFamily:"'Cormorant Garamond', serif", fontSize:22, fontWeight:700, color:C.textDark }}>Cycle Summary</h2>
              <button onClick={() => navigate("/tracker")} style={{ background:"none", border:"none", cursor:"pointer", fontFamily:"'Nunito', sans-serif", fontSize:13, fontWeight:700, color:C.coral }}>View tracker →</button>
            </div>
            <div style={{ display:"grid", gridTemplateColumns:"repeat(4, 1fr)", gap:12 }}>
              {[
                ...
                <div key={label} style={{ background:C.peachPale, borderRadius:16, padding:"16px", border:`1px solid ${C.peachLight}`, textAlign:"center" }}>
                  <p style={{ fontFamily:"'Cormorant Garamond', serif", fontSize:22, fontWeight:700, color:C.coral }}>{val}</p>
                  <p style={{ fontFamily:"'Nunito', sans-serif", fontSize:11, color:C.textSoft, marginTop:4 }}>{label}</p>
                </div>
              ))}
            </div>
          </div>
